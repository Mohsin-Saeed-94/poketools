...
class Encounter extends AbstractDoctrineDataMigration implements DataMigrationInterface
{
    ...
    public function transform($sourceData, $destinationData)
    {
        ...
        foreach ($location->getAreas() as $checkLocationArea) {
            if ($checkLocationArea->getSlug() == $locationSlug.'/'.$sourceData['area']) {
        ...
        $pokemon = null;
        $speciesSlug = $species->getSlug();
        foreach ($species->getPokemon() as $checkPokemon) {
            if ($checkPokemon->getSlug() == $speciesSlug.'/'.$sourceData['pokemon']) {
        ...
        if (isset($sourceData['conditions'])) {
            ...
            foreach ($conditions as &$condition) {
                ...
                $conditionSlug = $condition->getSlug();
                $state = null;
                foreach ($condition->getStates() as $checkState) {
                    if ($checkState->getSlug() == $conditionSlug.'/'.$stateIdentifier) {
                        $state = $checkState;
                        break;
                    }
                }
